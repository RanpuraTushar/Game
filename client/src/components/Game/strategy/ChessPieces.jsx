// ChessPieces.jsx - High-Definition Scalable Vector Graphics for Chess Pieces
import React from 'react';

export const ChessPiece = ({ type, color, className = '' }) => {
  const isWhite = color === 'w';
  const fill = isWhite ? '#ffffff' : '#1e2029';
  const stroke = isWhite ? '#2c3e50' : '#00f3ff';
  const glow = isWhite ? 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' : 'drop-shadow(0 0 6px rgba(0,243,255,0.6))';

  switch (type.toLowerCase()) {
    case 'p': // Pawn
      return (
        <svg className={`chess-piece-svg ${className}`} viewBox="0 0 45 45" style={{ filter: glow }}>
          <path
            d="m 22.5,9 c -2.21,0 -4,1.79 -4,4 0,0.89 0.29,1.71 0.78,2.38 C 17.33,16.5 16,18.59 16,21 c 0,2.03 0.94,3.84 2.41,5.03 C 15.41,27.09 11,31.58 11,39.5 l 23,0 c 0,-7.92 -4.41,-12.41 -7.41,-13.47 C 28.06,24.84 29,23.03 29,21 29,18.59 27.67,16.5 25.72,15.38 26.21,14.71 26.5,13.89 26.5,13 c 0,-2.21 -1.79,-4 -4,-4 z"
            fill={fill}
            stroke={stroke}
            strokeWidth="1.5"
            strokeLinecap="round"
          />
        </svg>
      );

    case 'n': // Knight
      return (
        <svg className={`chess-piece-svg ${className}`} viewBox="0 0 45 45" style={{ filter: glow }}>
          <path
            d="M 22,10 C 32.5,11 38.5,18 38,39 L 15,39 C 15,30 25,32.5 23,18"
            fill={fill}
            stroke={stroke}
            strokeWidth="1.5"
          />
          <path
            d="M 24,18 C 24.38,20.91 18.45,25.37 16,27 C 13,29 13.18,31.34 11,31 C 9.958,30.06 12.41,27.96 11,28 C 10,28 11.19,29.23 10,30 C 9,30 5.997,31 6,26 C 6,24 12,14 12,14 C 12,14 13.89,12.1 14,10.5 C 13.27,9.506 13.5,8.5 15,7.5 C 14.5,6 16.5,2 26,2 C 27,2 30.5,3.5 32,7.5 C 29,8 24.5,10.5 24,18 z"
            fill={fill}
            stroke={stroke}
            strokeWidth="1.5"
          />
          <circle cx="15" cy="11.5" r="1.5" fill={isWhite ? '#2c3e50' : '#00f3ff'} />
        </svg>
      );

    case 'b': // Bishop
      return (
        <svg className={`chess-piece-svg ${className}`} viewBox="0 0 45 45" style={{ filter: glow }}>
          <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round">
            <path d="M 9,36 C 12.39,35.03 19.11,36.43 22.5,34 C 25.89,36.43 32.61,35.03 36,36 C 36,36 37.65,36.54 39,38 C 38.32,38.97 37.35,38.99 36,38.5 C 32.61,37.53 25.89,38.96 22.5,37.5 C 19.11,38.96 12.39,37.53 9,38.5 C 7.646,38.99 6.677,38.97 6,38 C 7.354,36.54 9,36 9,36 z" />
            <path d="M 12,36 C 11,32 18.5,27.5 18.5,22 C 18.5,17 16,16 16,14 C 16,11.5 19,8.5 22.5,8.5 C 26,8.5 29,11.5 29,14 C 29,16 26.5,17 26.5,22 C 26.5,27.5 34,32 33,36 z" />
            <circle cx="22.5" cy="6.5" r="2.5" />
            <path d="M 17.5,26 L 27.5,26 M 15,30 L 30,30 M 22.5,15.5 L 22.5,20.5 M 20,18 L 25,18" stroke={isWhite ? '#2c3e50' : '#00f3ff'} />
          </g>
        </svg>
      );

    case 'r': // Rook
      return (
        <svg className={`chess-piece-svg ${className}`} viewBox="0 0 45 45" style={{ filter: glow }}>
          <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round">
            <path d="M 9,39 L 36,39 L 36,36 L 9,36 z" />
            <path d="M 12,36 L 12,32 L 33,32 L 33,36 z" />
            <path d="M 11,14 L 11,9 L 15,9 L 15,11 L 20,11 L 20,9 L 25,9 L 25,11 L 30,11 L 30,9 L 34,9 L 34,14 z" />
            <path d="M 34,14 L 31,17 L 14,17 L 11,14 z" />
            <path d="M 14,17 L 14,32 L 31,32 L 31,17 z" />
          </g>
        </svg>
      );

    case 'q': // Queen
      return (
        <svg className={`chess-piece-svg ${className}`} viewBox="0 0 45 45" style={{ filter: glow }}>
          <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round">
            <path d="M 9,26 C 17.5,24.5 30,24.5 36,26 L 38.5,13.5 L 31,25 L 22.5,10 L 14,25 L 6.5,13.5 z" />
            <path d="M 9,26 C 9,28 10.5,28 11.5,30 C 12.5,31.5 12.5,31 12,33.5 C 10.5,34.5 11,36 11,36 C 9.5,37.5 11,38.5 11,38.5 L 34,38.5 C 34,38.5 35.5,37.5 34,36 C 34,36 34.5,34.5 33,33.5 C 32.5,31 32.5,31.5 33.5,30 C 34.5,28 36,28 36,26 z" />
            <circle cx="6" cy="12" r="2" />
            <circle cx="14" cy="9" r="2" />
            <circle cx="22.5" cy="8" r="2" />
            <circle cx="31" cy="9" r="2" />
            <circle cx="39" cy="12" r="2" />
          </g>
        </svg>
      );

    case 'k': // King
      return (
        <svg className={`chess-piece-svg ${className}`} viewBox="0 0 45 45" style={{ filter: glow }}>
          <g fill={fill} stroke={stroke} strokeWidth="1.5" strokeLinecap="round">
            <path d="M 22.5,11.63 L 22.5,6" />
            <path d="M 20,8 L 25,8" />
            <path d="M 22.5,25 C 22.5,25 27,17.5 25.5,14.5 C 24,11.5 21,11.5 19.5,14.5 C 18,17.5 22.5,25 22.5,25" />
            <path d="M 11.5,37 C 17,40.5 28,40.5 33.5,37 C 36.5,33.5 36.5,25 35,22 C 30,17 15,17 10,22 C 8.5,25 8.5,33.5 11.5,37 z" />
            <path d="M 20,27.5 L 25,27.5 M 19,31 L 26,31 M 17,34.5 L 28,34.5" stroke={isWhite ? '#2c3e50' : '#00f3ff'} />
          </g>
        </svg>
      );

    default:
      return null;
  }
};
